...
C_ACUMINATA = Species(
    ...
    known_for=[{EN: "kola nuts"}],
)
M_SYLVESTRIS = Species(
    name="Malva sylvestris", local_names={PL: "ślaz dziki", EN: "common mallow"}
)
H_SABDARIFFA = Species(
    name="Hibiscus sabdariffa",
    local_names={EN: "roselle", PL: "ketmia szczawiowa"},
    known_for=[
        {EN: "hibiscus tea", PL: "herbata z hibiskusa (częsta baza herbat owocowych)"}
    ],
)
A_ROSEA = Species(
    name="Alcea rosea", local_names={EN: "common hollyhock", PL: "malwa różowa"}
)
A_DIGITATA = Species(
...
D_ZIBETHINUS = Species(
    name="Durio zibethinus", local_names={EN: "durian", PL: "durian właściwy"}
)
