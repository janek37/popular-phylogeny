from clade import Order, Clade, Family, Subfamily, Genus, Species
from constants import EN, PL

O_MILITARIS = Species(name='Orchis militaris', local_names={EN: 'military orchid', PL: 'storczyk kukawka'})
...
I_SIBRICA = Species(name='Iris sibirica', local_names={EN: 'Siberian iris', PL: 'kosaciec syberyjski'})
C_SATIVUS = Species(
    name='Crocus sativus',
    local_names={EN: 'saffron crocus', PL: 'szafran uprawny'},
    known_for=[{EN: 'saffron', PL: 'szafran (przyprawa)'}]
)
C_VERNUS = Species(name='Crocus vernus', local_names={EN: 'spring crocus', PL: 'krokus/szafran wiosenny'})
A_OFFICINALIS = Species(name='Asparagus officinalis', local_names={EN: 'garden asparagus', PL: 'szparag lekarski'})
A_AMERICANA = Species(name='Agave americana', local_names={EN: 'century plant, maguey', PL: 'agawa amerykańska'})
A_TEQUILANA = Species(
    name='Agave tequilana',
    local_names={EN: 'blue agave', PL: 'agawa niebieska'},
    known_for=[{EN: 'tequila'}],
)
C_MAJALIS = Species(name='Convallaria majalis', local_names={EN: 'lily of the valley', PL: 'konwalia majowa'})
H_ORIENTALIS = Species(name='Hyacinthus orientalis', local_names={EN: 'common hyacinth', PL: 'hiacynt wschodni'})
...
A_SCHOENOPRASUM = Species(name='Allium schoenoprasum', local_names={EN: 'chives', PL: 'szczypiorek'})
A_URSINUM = Species(name='Allium ursinum', local_names={PL: 'czosnek niedźwiedzi', EN: 'wild garlic'})
N_POETICUS = Species(name='Narcissus poeticus', local_names={PL: 'narcyz biały', EN: "poet's daffodil"})
G_NIVALIS = Species(name='Galanthus nivalis', local_names={PL: 'snieżyczka przebiśnieg', EN: 'common snowdrop'})
...
ASPARAGACEAE = Family(name='Asparagaceae', children=[ASPARAGOIDEAE_NOLINOIDEAE, AGAVOIDEAE_SCILLOIDEAE])
