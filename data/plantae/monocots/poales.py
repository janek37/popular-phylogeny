from clade import Clade, Species, Genus, Family, Order, Tribe, Subfamily, Subtribe, Supertribe
...
A_COMOSUS = Species(name='Ananas comosus', local_names={EN: 'pineapple', PL: 'ananas jadalny'})
P_PRATENSIS = Species(
    name='Poa pratensis',
    local_names={EN: 'Kentucky bluegrass, smooth meadow-grass', PL: 'wiechlina łąkowa'},
)
F_OVINA = Species(name='Festuca ovina', local_names={EN: 'sheep fescue', PL: 'kostrzewa owcza'})
T_AESTIVUM = Species(
...
H_VULGARE = Species(name='Hordeum vulgare', local_names={EN: 'barley', PL: 'jęczmień zwyczajny'})
A_SATIVA = Species(name='Avena sativa', local_names={EN: 'oat', PL: 'owies zwyczajny'})
S_CEREALE = Species(name='Secale cereale', local_names={EN: 'rye', PL: 'żyto zwyczajne'})
O_SATIVA = Species(name='Oryza sativa', local_names={EN: 'rice, Asian rice', PL: 'ryż siewny'})
...
S_BICOLOR = Species(name='Sorghum bicolor', local_names={PL: 'sorgo dwubarwne', EN: 'sorghum'})
S_OFFICINARUM = Species(
    name='Saccharum officinarum',
    local_names={PL: 'trzcina cukrowa, cukrowiec lekarski', EN: 'sugarcane'},
)
P_AUSTRALIS = Species(name='Phragmites australis', local_names={PL: 'trzcina pospolita', EN: 'common reed'})
...
ANDROPOGONEAE = Tribe(name='Andropogoneae', children=[TRIPSACINAE, ANDROPOGONINAE_SACCHARINAE])
