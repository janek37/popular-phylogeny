...
N_ALBA = Species(
    name='Nymphaea alba',
    local_names={EN: 'European white water lily, white nenuphar', PL: 'lilia wodna, nenufar, grzybiebie białe'},
)
I_VERUM = Species(
    name='Illicium verum',
...
M_VIRGINIANA = Species(name='Magnolia virginiana', local_names={EN: 'sweetbay magnolia'})
M_FRAGRANS = Species(name='Myristica fragrans', known_for=[{PL: 'gałka muszkatołowa'}, {EN: 'nutmeg'}])
L_NOBILIS = Species(
    name='Laurus nobilis',
    local_names={EN: 'laurel', PL: 'wawrzyn szlachetny'},
    known_for=[{EN: 'bay leaf', PL: 'liść laurowy'}, {EN: 'laurel wreath', PL: 'wieniec laurowy'}],
)
C_VERUM = Species(name='Cinnamomum verum', known_for=[{EN: 'cinnamon', PL: 'cynamon'}])
C_CAMPHORA = Species(name='Cinnamomum camphora', known_for=[{EN: 'camphor', PL: 'kamfora'}])
P_AMERICANA = Species(name='Persea americana', local_names={EN: 'avocado', PL: 'awokado'})
P_NIGRUM = Species(name='Piper nigrum', local_names={EN: 'black pepper', PL: 'pieprz czarny'})
