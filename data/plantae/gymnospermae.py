from clade import Clade, Species, Family, Order, Class, Phylum, Genus, Subfamily
from constants import EN, PL

A_ALBA = Species(name='Abies alba', local_names={EN: 'silver fir', PL: 'jodła pospolita'})
P_SILVESTRIS = Species(name='Pinus sylvestris', local_names={EN: 'European red pine', PL: 'sosna zwyczajna'})
P_ABIES = Species(name='Picea abies', local_names={EN: 'European spruce', PL: 'świerk pospolity'})
L_DECIDUA = Species(name='Larix decidua', local_names={EN: 'European larch', PL: 'modrzew europejski'})
C_LIBANI = Species(name='Cedrus libani', local_names={EN: 'Lebanese cedar', PL: 'cedr libański'})
...
J_COMMUNIS = Species(name='Juniperus communis', local_names={PL: 'jałowiec pospolity', EN: 'common juniper'})
S_GIGANTEUM = Species(
...
T_BACCATA = Species(name='Taxus baccata', local_names={PL: 'cis pospolity', EN: 'common yew'})
G_BILOBA = Species(name='Ginkgo biloba', local_names={PL: 'miłorząb dwuklapowy', EN: 'ginkgo'})
...
CUPRESSALES = Clade(name='Cupressales', children=[CUPRESSACEAE, TAXACEAE])  # former Order
