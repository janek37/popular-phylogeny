from clade import Kingdom, Phylum, Class, Order, Family, Genus, Species, Clade

# Penicillium
from constants import EN, PL
from data.fungi.mushrooms import BASIDIOMYCOTA

S_CHARTARUM = Species(name='Stachybotrys chartarum', local_names={EN: 'toxic black mold'})
A_ORYZAE = Species(name='Aspergillus oryzae', known_for=[{EN: 'sake'}, {EN: 'soy sauce'}, {EN: 'miso'}])
P_CAMEMBERTI = Species(name='Penicillium camemberti', known_for=[{EN: 'Camembert'}, {EN: 'Brie'}])
P_CHRYSOGENUM = Species(name='Penicillium chrysogenum', known_for=[{EN: 'penicilin'}])
P_DIGITATUM = Species(
    name='Penicillium digitatum',
    known_for=[{EN: 'blue mold of citrus', PL: 'niebieska zgnilizna owoców cytrusowych'}],
)
P_EXPANSUM = Species(
    name='Penicillium expansum', known_for=[{EN: 'blue mold of apples', PL: 'mokra zgnilizna jabłek'}]
)
P_ROQUEFORTI = Species(name='Penicillium roqueforti', known_for=[{EN: 'Roquefort'}])
S_CEREVISIAE = Species(
    name='Saccharomyces cerevisiae',
    known_for=[{EN: 'ale', PL: 'piwo górnej fermentacji'}, {EN: "baker's yeast", PL: 'drożdże piekarskie'}],
)
S_PASTORIANUS = Species(name='Saccharomyces pastorianus', known_for=[{EN: 'lager', PL: 'piwo dolnej fermentacji'}])
C_ALBICANS = Species(name='Candida albicans', known_for=[{EN: 'candidiasis', PL: 'drożdżyca, kandydoza'}])
H_LACTIFLUORUM = Species(name='Hypomyces lactifluorum', local_names={EN: 'lobster mushroom'})
M_ESCULENTA = Species(name='Morchella esculenta', local_names={EN: 'morel mushroom', PL: 'smardz jadalny'})
T_MAGNATUM = Species(name='Tuber magnatum', local_names={EN: 'white truffle', PL: 'trufla biała'})
C_ARBUSCULA = Species(name='Cladonia arbuscula', local_names={EN: 'shrubby cup lichen', PL: 'chrobotek leśny'})
F_CAPERATA = Species(name='Flavoparmelia caperata', local_names={EN: 'common greenshield lichen'})
...
PENICILLIUM = Genus(name='Penicillium', children=[P_CAMEMBERTI, P_CHRYSOGENUM, PENICILLIUM_SECTION, P_ROQUEFORTI])
...
SACCHAROMYCETACEAE = Family(name='Saccharomycetaceae', children=[SACCHAROMYCES, CANDIDA])
...
DOTHIDEOMYCETA = Clade(name='Dothideomyceta', children=[EUROTIOMYCETES, LECANOROMYCETES])
LEOTIOMYCETA = Clade(name="Leotiomyceta", children=[SORDARIOMYCETES, DOTHIDEOMYCETA])
PEZIZOMYCOTINA = Clade(name='Pezizomycotina', children=[LEOTIOMYCETA, PEZIZOMYCETES])  # Subdivision/Subphylum
